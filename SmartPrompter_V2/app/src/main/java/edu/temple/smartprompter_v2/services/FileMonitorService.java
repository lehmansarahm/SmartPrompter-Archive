...
public class FileMonitorService extends Service {
    ...
    private void doWork() {
        ...
        timer.scheduleAtFixedRate(new TimerTask() {
            @Override
            public void run() {
                boolean isAlarmsFolderDirty = StorageUtil.getDirtyStatus(FileMonitorService.this);
                Log.i(LOG_TAG, "FileMonitorService - Is Alarms folder dirty:  " + isAlarmsFolderDirty);
                if (isAlarmsFolderDirty) ((SmartPrompter)getApplicationContext()).cleanupDirtyAlarms();
            }
        }, 0, JOB_INTERVAL);
    }
